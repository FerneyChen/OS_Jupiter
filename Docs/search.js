        ...
        for (i = 0; i < resulttitlecache.length; i++) {
            for (j = 0; j < obj.length; j++) {
                if (obj[j]['title'] == resulttitlecache[i]) {
                    titlesearchresult = '<a href="https://offsaying.com/OS_Jupiter/Docs/' + obj[j]["path"] + '" style="font-family:Poppins;color:#f0e68c;font-size:18px">' + obj[j]['title'].replace(new RegExp(input.value, 'g'), '<mark style="background-color:transparent;color:#00ffff">' + input.value + '</mark>')+' <i class="ri-check-double-line"></i><br>'
